...
class AttachmentMapper extends MapperAbstract
{
    ...
    protected function _map($record)
    {
        ...
        $attachment->byStaff = (boolean) $record['by_staff'];
    ...
    public function save(Attachment $attachment)
    {
        ...
        if (!empty($attachment->attachmentId)) {
            ...
            $query .= ' file_id = :fileId, video_id = :videoId, by_staff = :byStaff, date_created = :dateCreated';
            $query .= ' WHERE attachment_id = :attachmentId';
            $bindParams = array(
                ':attachmentId' => $attachment->attachmentId,
                ':file_id' => $attachment->fileId,
                ':video_id' => $attachment->videoId,
                ':byStaff' => isset($attachment->byStaff) ? $attachment->byStaff : false,
            ...
            $query .= ' (file_id, video_id, by_staff, date_created)';
            $query .= ' VALUES (:fileId, :videoId, :byStaff, :dateCreated)';
            $bindParams = array(
